...
const K100_STATS = [
  { value: '9.6 km', label: 'Pilot Stretch', desc: 'Desilted & restored channel' },
  { value: '32 km²', label: 'Catchment Area', desc: 'Stormwater catchment basin' },
  { value: '~₹175 cr', label: 'Project Cost', desc: 'Coordinated funding budget' },
  { value: '15', label: 'Agencies', desc: 'Government bodies aligned' },
  { value: '135 → 5 MLD', label: 'Sewage Inflow', desc: 'Over 96% sewage diverted' }
];
...
const K100_ASSETS = {
  publicRealm: [
    { title: 'Restored channel', desc: '9.6 km of desilted, re-graded waterway with restored gravity flow.' },
    { title: 'Walkway & promenade', desc: 'Pedestrian paths along the channel edge — the new public space.' },
    { title: 'Bridges & culverts', desc: 'Redesigned arched crossings that widen flow and let people cross.' },
    { title: 'Stone edges & planting', desc: 'Native granite edging and indigenous species as a living bio-filter.' },
    { title: 'Seating & park space', desc: 'Plazas, seating and Bengaluru’s first new park in nearly four decades.' },
    { title: 'Wayfinding & signage', desc: 'A dedicated identity and signage system for the corridor.' },
    { title: 'Lighting', desc: 'New lighting that made the edge feel safe and usable after dark.' }
  ],
  hydraulicSystem: [
    { title: '5 MLD STP', desc: 'Treatment plant near KR Market supplying clean dry-weather flow.' },
    { title: 'Decentralised treatment', desc: 'On-site wastewater treatment serving the D’Souza Garden settlement.' },
    { title: 'Trunk & interceptor sewers', desc: 'New / rectified lines (Chickpet–NGV; Hosur Rd–Neelasandra) diverting sewage away.' },
    { title: 'Pumping stations', desc: 'Intermediate stations, incl. a 15 MLD wastewater pump on KSRTC land.' },
    { title: 'Compact substations', desc: 'BESCOM CSS units replacing exposed overhead lines along the corridor.' },
    { title: 'Sensor network', desc: 'KSNDMC flood-line and choke-point sensors for risk monitoring.' },
    { title: 'Demonstration stretch', desc: 'The ~300 m Shanthinagar mock-up that de-risked the full build.' }
  ]
};
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-[24px] p-6 md:p-8 flex flex-col gap-6 shadow-sm text-left">
      <div className="text-left">
        ...
        <p className="text-[13.5px] text-[#6E6455] m-0 leading-relaxed">
      ...
      {/* Tab switcher */}
      <div className="flex gap-2 border-b-2 border-[#C8D7BC]/40 pb-0.5 overflow-x-auto">
        ...
        {K100_STATS.map((stat, idx) => (
          <div key={idx} className="bg-white border border-[#C8D7BC]/80 rounded-2xl p-4.5 flex flex-col justify-between shadow-sm hover:-translate-y-0.5 hover:shadow-md transition-all duration-200 text-left">
            <div className="flex flex-col gap-0.5">
              ...
              <div className="text-[11px] text-[#6E6455] mt-1.5 leading-snug">{stat.desc}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white border border-[#C8D7BC]/80 rounded-[24px] p-6 md:p-8 flex flex-col gap-6 shadow-sm text-left">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {K100_ASSETS[selectedAssetTab].map((asset, idx) => (
            <div key={idx} className="bg-[#C8D7BC]/10 border border-[#C8D7BC]/60 rounded-2xl p-4 flex flex-col gap-1 transition-all duration-200 hover:-translate-y-0.5 hover:border-[#347745]/40 hover:bg-white hover:shadow-md text-left">
              <h4 className="text-[13.5px] font-bold text-[#1F2A24] m-0">{asset.title}</h4>
              <p className="text-[12.5px] leading-relaxed text-[#6E6455] m-0">{asset.desc}</p>
            </div>
          ))}
        </div>
      </div>
