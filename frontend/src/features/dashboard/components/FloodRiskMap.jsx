...
const FloodRiskMap = () => {
  ...
  return (
    <div className="flex flex-col gap-8 animate-[fadeInUp_0.4s_ease_both]">
      ...
      <div className="flex gap-2 bg-[#C8D7BC]/20 border border-[#C8D7BC]/50 p-1.5 rounded-xl self-start flex-wrap">
        <button 
          className={`px-4 py-2 border-none rounded-lg text-[13.5px] font-bold transition-all duration-200 cursor-pointer ${activeSubTab === 'all' ? 'bg-[#347745] text-white shadow-sm' : 'bg-transparent text-[#6E6455] hover:text-[#1F2A24]'}`}
          onClick={() => { setActiveSubTab('all'); }}
        >
          All risk levels
        </button>
        <button 
          className={`px-4 py-2 border-none rounded-lg text-[13.5px] font-bold transition-all duration-200 cursor-pointer ${activeSubTab === 'high' ? 'bg-[#347745] text-white shadow-sm' : 'bg-transparent text-[#6E6455] hover:text-[#1F2A24]'}`}
          onClick={() => { setActiveSubTab('high'); }}
        >
          High risk
        </button>
        <button 
          className={`px-4 py-2 border-none rounded-lg text-[13.5px] font-bold transition-all duration-200 cursor-pointer ${activeSubTab === 'medium' ? 'bg-[#347745] text-white shadow-sm' : 'bg-transparent text-[#6E6455] hover:text-[#1F2A24]'}`}
          onClick={() => { setActiveSubTab('medium'); }}
        >
          Medium risk
        </button>
        <button 
          className={`px-4 py-2 border-none rounded-lg text-[13.5px] font-bold transition-all duration-200 cursor-pointer ${activeSubTab === 'zones' ? 'bg-[#347745] text-white shadow-sm' : 'bg-transparent text-[#6E6455] hover:text-[#1F2A24]'}`}
          onClick={() => { setActiveSubTab('zones'); }}
        >
          Flood zones
        </button>
        <button 
          className={`px-4 py-2 border-none rounded-lg text-[13.5px] font-bold transition-all duration-200 cursor-pointer ${activeSubTab === 'waterlogging' ? 'bg-[#347745] text-white shadow-sm' : 'bg-transparent text-[#6E6455] hover:text-[#1F2A24]'}`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-10 items-start">
        {/* Left Map Panel */}
        <div className="bg-white border border-[#C8D7BC]/80 rounded-[24px] p-6 shadow-sm">
          <div className="flex justify-between items-center border-b border-[#C8D7BC]/40 pb-3.5 mb-5">
            <h4 className="text-base font-bold text-[#1F2A24] m-0">Bengaluru flood risk map</h4>
            <span className="text-xs text-[#6E6455] font-medium">Last updated Jan 2025</span>
          </div>

          <div className="bg-[#C8D7BC]/20 border border-[#C8D7BC]/60 rounded-2xl p-3 overflow-hidden shadow-inner flex justify-center items-center">
        ...
        <div className="lg:sticky lg:top-[100px] w-full">
          {selectedZone ? (
            <div className={`bg-white rounded-[24px] p-8 border border-[#C8D7BC]/80 shadow-sm flex flex-col gap-5 border-t-[6px] ${selectedZone.riskLevel === 'high' ? 'border-t-red-500' : selectedZone.riskLevel === 'medium' ? 'border-t-orange-500' : 'border-t-yellow-500'}`}>
              <div className="flex justify-between items-center border-b border-[#C8D7BC]/40 pb-4">
                <h3 className="text-lg font-bold text-[#1F2A24] m-0 tracking-tight">{selectedZone.name}</h3>
              ...
              <div className="flex flex-col gap-5">
                <p className="text-[13.5px] leading-relaxed text-[#6E6455] m-0 text-left">{selectedZone.details}</p>
                
                <div className="flex flex-col gap-2.5 bg-[#C8D7BC]/10 p-4 border border-[#C8D7BC]/50 rounded-xl">
                  <div className="flex justify-between items-center text-xs md:text-sm">
                    <span className="text-[#6E6455] font-medium">Wards Affected:</span>
                    <strong className="text-[#1F2A24] font-semibold">{selectedZone.wardsCount} Wards</strong>
                  </div>
                  <div className="flex justify-between items-center text-xs md:text-sm">
                    <span className="text-[#6E6455] font-medium">BGG Interventions:</span>
                    <strong className="text-[#1F2A24] font-semibold">{selectedZone.activeInterventions} Sites</strong>
                  </div>
                </div>

                <div className="flex flex-col gap-2">
                  <h5 className="text-xs font-bold text-[#1F2A24] m-0 uppercase tracking-wider text-left">🛡️ Risk Management Strategy</h5>
